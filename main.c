...
int read_from_socket(char* requestBuffer, int bufferSize)
{
    int addr_len = sizeof(socket_addr);
    int newsockFD = accept(tcp_socket, (struct sockaddr*)&socket_addr, (socklen_t*)&addr_len);
    if (newsockFD == -1) {
        perror("accept");
        return 1;
    }
    ...
    int valRead = recv(newsockFD, requestBuffer, bufferSize, 0);
    if (valRead == -1) {
        perror("read");
        return 1;
    }
...
int open_socket_connection()
{
    ...
    SOCKET tcp_socket = socket(result->ai_family, result->ai_socktype, result->ai_protocol);
...
SOCKET read_from_socket(char* requestBuffer, int bufferSize)
{
    ...
    int valRead = recv(clientSocket, requestBuffer, bufferSize, 0);
    if (valRead == -1) {
        perror("read");
        return 1;
    }
...
int main()
{
    ...
    for (;;) {
        ...
        int newsockFD = read_from_socket(request, MAXBUFLEN);
